<template>
  <div class="item_detail">
    <van-swipe :autoplay="2000">
      <van-swipe-item v-for="(image, index) in goods.info.gallery" :key="image">
  ...
  <div class="item_cell_group">
    <van-cell-group>
      ...
      <van-cell title="运费" :value="goods.info.freightRule"/>
    </van-cell-group>
    <van-sku
      v-model="showSku"
      :sku="sku"
...
import { goodsDetail, cartGoodsCount, collectAddOrDelete, cartAdd, checkProducts } from '@/api/api';
...
export default {
  ...
  computed: {
    props_str() {
      ...
      _.each(this.goods.attribute, json => {
        props_arr.push([json['extensionAttribute'], json['extensionAttributeValue']]);
      });
      return props_arr || [];
    }
  },
  ...
  methods: {
    ...
    getProductId(s1, s2) {
      var productId;
      for(var i=0;i<this.goods.productList.products.length;i++){
          if (this.goods.productList.products[i].xlggz==s1+','+s2){
              productId=this.goods.productList.products[i].spkcbm;
              break;
          }
      }
      return productId;
    ...
    setSkuList() {
      var sku_list = [];
      var products=this.goods.productList.products;
      for(var i=0;i<products.length;i++){
          var sku_list_obj={}
          var split = products[i].xlggz.split(',');
          switch (split.length) {
              case 1:
                  sku_list_obj.s1=split[0];
                  sku_list_obj.id=products[i].spkcbm;
                  break;
              case 2:
                  sku_list_obj.s1=split[0];
                  sku_list_obj.s2=split[1];
                  sku_list_obj.id=products[i].spkcbm;
                  break;
              // case 3:
              //     sku_list_obj.s1=split[0]
              //     sku_list_obj.s2=split[1]
              //     sku_list_obj.s3=split[2]
          }
          sku_list_obj.price=products[i].spsj*100
          sku_list_obj.stock_num = products[i].kc
    ...
    findSpecValueIdByName(name) {
      ...
      _.each(this.goods.specificationList, specification => {
        _.each(specification.valueList, specValue => {
          if (specValue.ggz === name) {
            id = specValue.spggsxzbm;
            return;
          }
        });
        if (id !== 0) {
    ...
    setSkuTree() {
      var _specifications=this.goods.productList.specifications
      var specificationSet=new Set()
      var specificationMap=new Map()
      for(var i=0;i<_specifications.length;i++){
          specificationSet.add(_specifications[i].ggmc)
      }
      ...
      for(var i=0;i<_specifications.length;i++){
          var temp=specificationMap.get(_specifications[i].ggmc)
          temp.push(_specifications[i].ggz)
          specificationMap.set(_specifications[i].ggmc,temp)
      }
        var tempspecifi=Array.from(specificationMap)
        let specifications = [];
      for(var i=0;i<tempspecifi.length;i++){
          let values=[];
          for(var j=0;j<tempspecifi[i][1].length;j++){
              values.push({
                  id:tempspecifi[i][1][j],
                  name:tempspecifi[i][1][j]
              })
          }
          specifications.push({
              k:tempspecifi[i][0],
