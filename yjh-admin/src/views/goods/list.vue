<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="市场价格" prop="retailPrice"/>

      <el-table-column align="center" label="当前价格" prop="counterPrice"/>
      ...
      <el-table-column align="center" label="是否是推荐商品" prop="isRecommendation">
        <template slot-scope="scope">
          <el-tag :type="scope.row.isRecommendation ? '1' : '0' ">{{ scope.row.isRecommendation ? '是' : '否' }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" label="是否在售" prop="isOnSale">
        <template slot-scope="scope">
          <el-tag :type="scope.row.isOnSale ? '1' : '0' ">{{ scope.row.isOnSale ? '在售' : '未售' }}</el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.$router.push({ path: '/goods/edit', query: { id: row.goodsId }})
    },
    showDetail(detail) {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        const tHeader = ['商品ID', '商品编号','名称','市场价格', '当前价格','商品单位','品牌商ID', '是否在售']
        const filterVal = ['spbm','spbm','spmc','scj','spsj','jldw','gysbm','sfsj','cpms']
        excel.export_json_to_excel2(tHeader, this.list, filterVal)
