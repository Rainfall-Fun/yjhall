<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="品牌商ID" prop="spppbm"/>

      <el-table-column align="center" label="品牌商名称" prop="ppmc"/>

      <el-table-column align="center" label="品牌商网址" prop="ppwz"/>
      <el-table-column align="center" label="显示排序" prop="xspx">
        <template slot-scope="scope">
          <el-tag :type="scope.row.xspx ? '1' : '0' ">{{ scope.row.xspx ? '是' : '否' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" label="是否显示" prop="sfxs">
        <template slot-scope="scope">
          <el-tag :type="scope.row.sfxs ? '1' : '0' ">{{ scope.row.sfxs ? '是' : '否' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="400px" label="介绍" prop="ppjj"/>
      <el-table-column align="center" label="添加人" prop="tjr"/>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="品牌商名称" prop="ppmc">
          <el-input v-model="dataForm.ppmc"/>
        </el-form-item>
        <el-form-item label="品牌商网址" prop="ppwz">
          <el-input v-model="dataForm.ppwz"/>
        </el-form-item>
        <el-form-item label="显示排序" prop="xspx">
          <el-select v-model="dataForm.xspx">
            <el-option label="是" value=1 />
            <el-option label="否" value=0 />
          </el-select>
        </el-form-item>
        <el-form-item label="是否显示" prop="sfxs">
          <el-select v-model="dataForm.sfxs">
            <el-option label="是" value=1 />
            <el-option label="否" value=0 />
          </el-select>
        </el-form-item>
        <el-form-item label="介绍" prop="ppjj">
          <el-input v-model="dataForm.ppjj"/>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      row.sfxs=String(row.sfxs)
      row.xspx=String(row.xspx)
            ...
            .then(() => {
              for (const v of this.list) {
                if (v.id === this.dataForm.id) {
                  ...
                  this.dataForm.sfxs=Number(this.dataForm.sfxs)
                  this.dataForm.xspx=Number(this.dataForm.xspx)
